  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch("/import-excel", {
        method: "POST",
        body: formData,
        // Khi làm việc với React và Spring Boot, bạn có thể không cần header 'Content-Type'
        // bởi vì 'fetch' sẽ tự đặt 'Content-Type' phù hợp dựa trên body của FormData
      });

      if (response.ok) {
        const result = await response.json();
        console.log("File tải lên thành công:", result);
        alert("File đã được tải lên thành công.");
      } else {
        // Xử lý response khi kết quả không như mong đợi
        console.error("Lỗi khi tải file:", response);
        alert("Có lỗi xảy ra khi tải file.");
      }
    } catch (error) {
      console.error("Lỗi ngoại lệ:", error);
      alert("Lỗi ngoại lệ xảy ra khi tải file.");
    }
  };
// 
  useEffect(() => {
        ...
        <div className="relative p-4 w-full max-w-md max-h-full">
          ...
          <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            {/* Modal body */}
                ...
                <form className="p-4 md:p-5" method="POST"  encType="multipart/form-data">
                  <div className="grid gap-4 mb-4 grid-cols-2">
                    <div className="col-span-2">
                      <label
                        htmlFor="name"
