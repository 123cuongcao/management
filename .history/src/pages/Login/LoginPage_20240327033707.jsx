...
function LoginPage() {
  ...
  const history = unstable_HistoryRouter(); // Nếu bạn đang sử dụng React Router v6, thay vì useHistory(), sử dụng useNavigate().

  useEffect(() => {
    if (status === 'succeeded') {
      history.push('/StudentManagement'); // Hoặc window.location.href = '/StudentManagement';
    }
  }, [status, history]);
  ...
  const handleSubmit = (e) => {
    ...
    .unwrap()
    .then(() => {
      history.push('/StudentManagement'); // Chuyển hướng người dùng mà không tải lại trang
    })
    .catch(() => {
  ...
  return (
    ...
    <div className="flex gap-5 max-md:flex-col max-md:gap-0">
      ...
      <section className="flex flex-col ml-5 w-[46%] max-md:ml-0 max-md:w-full">
        <form  onSubmit={handleSubmit} className="flex flex-col self-stretch my-auto text-sm font-medium text-gray-600 max-md:mt-10">
