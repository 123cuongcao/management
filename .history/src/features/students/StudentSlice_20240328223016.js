...
export const addStudentFromExcel = createAsyncThunk(
  "student/addStudentFromExcel",
  async (file, thunkApi) => {
    const formData = new FormData();
    formData.append("excelFile", file);
    const res = await axios.post("/upload-excel", formData).then((res) => res.data);
    if (!Array.isArray(res)) throw new Error("Invalid excel format");
    for (let i = 0; i < res.length; i++) {
      // eslint-disable-next-line no-await-in-loop
      await thunkApi.dispatch(addStudent({ ...res[i] }));
    }
    return res;
)
